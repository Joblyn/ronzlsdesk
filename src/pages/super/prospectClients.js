...
export default function InActiveClients() {
  ...
  const getRows = data => {
    let rows = [];
    let active = data.filter(client => client.accountStatus === 'prospect');
  ...
  return (
    <Page
      title="Dropdowns"
      breadcrumbs={[{ name: 'Clients/Prospect', active: true }]}
    >
      {adminGetClient.users.filter(cl => cl.accountStatus === 'prospect')
        .length ?
        <>
          <ExcelTable
            exportData={adminGetClient.users.filter(
              cl => cl.accountStatus === 'prospect',
            )}
            id="clients"
            fileName="clientsFile"
            className="m-2"
            exportHead="PROSPECT CLIENTS"
          />
          <div
            style={{
              overflowX: 'auto',
